...
function deleteEstimate() {
    var studentForDelete = $('#studentForDelete').val();
    $.ajax({
        type: "GET",
        url: "/estimate/search/deleteByStudent?student=" + studentForDelete.replace("/ /g", "%20"),
...
function addNewEstimate() {
    ...
    var requestJSONparametr = "{\"year\": \"" + year + "\", \"semester\": \"" + semester + "\"," +
        " \"student\": \"" + student + "\", \"discipline\": \"" + discipline + "\", \"estimate\": \"" + estimate + "\"}";
    $.ajax({
        type: "POST",
        url: "/estimate/add",
        contentType: "application/json",
...
function getEstimates() {
    ...
    priceRequest.open("GET", "/estimate/", true);   //Указываем адрес GET-запроса
    priceRequest.onload = function (){             //Функция которая отправляет запрос на сервер для получения всех студентов
        ...
        parsedItem._embedded.estimate.forEach(function(item)  {
            ...
            priceElement.innerHTML = item['student'];
            ...
            dateOfPass.innerHTML = item['estimate'];
